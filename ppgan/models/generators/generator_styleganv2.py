...
class ModulatedConv2D(nn.Layer):
    ...
    def forward(self, inputs, style, weight=None):
        batch, in_channel, height, width = inputs.shape
        if style is None and weight is not None: style = self.modulation(weight)

        elif self.need_modulate: style = self.modulation(style)
...
class StyleGANv2Generator(nn.Layer):
    ...
    def get_mean_style_S(self, n_sample=10, n_latent=1024):
        mean_style = None
        for i in range(n_sample):
            style = self.mean_latent_s(n_latent)
            if mean_style is None:
                mean_style = style
            else:
                mean_style += style
    ...
    def style_affine(self, latent):
        if latent.ndim < 3: latent = latent.unsqueeze(1).tile((1, self.n_latent, 1))
        latent_ = []
    ...
    def synthesis(self, latent, noise=None, randomize_noise=True):
        ...
        if noise is None:
            if randomize_noise:
                noise = [None] * self.num_layers #[paddle.randn(getattr(self.noises, f"noise_{i}").shape) for i in range(self.num_layers)]#
            else:
                noise = [
        ...
        if truncation < 1.0:
            ...
            for style in styles:
                if truncation_cutoff is None:
                    ...
                    style[:,:truncation_cutoff] = truncation_latent[:,:truncation_cutoff] + truncation * (style[:,:truncation_cutoff] - truncation_latent[:,:truncation_cutoff])
